// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.29.0
// source: user.sql

package query

import (
	"context"

	"github.com/google/uuid"
)

const deleteUser = `-- name: DeleteUser :exec
UPDATE users SET is_deleted = true, updated_at = now() WHERE guid = $1
`

func (q *Queries) DeleteUser(ctx context.Context, guid uuid.UUID) error {
	_, err := q.exec(ctx, q.deleteUserStmt, deleteUser, guid)
	return err
}

const getUser = `-- name: GetUser :one
SELECT guid, name, email, occupation, is_deleted, created_at, updated_at FROM users WHERE guid = $1
`

func (q *Queries) GetUser(ctx context.Context, guid uuid.UUID) (User, error) {
	row := q.queryRow(ctx, q.getUserStmt, getUser, guid)
	var i User
	err := row.Scan(
		&i.Guid,
		&i.Name,
		&i.Email,
		&i.Occupation,
		&i.IsDeleted,
		&i.CreatedAt,
		&i.UpdatedAt,
	)
	return i, err
}

const getUserByEmail = `-- name: GetUserByEmail :one
SELECT guid, name, email, occupation, is_deleted, created_at, updated_at FROM users WHERE email = $1
`

func (q *Queries) GetUserByEmail(ctx context.Context, email string) (User, error) {
	row := q.queryRow(ctx, q.getUserByEmailStmt, getUserByEmail, email)
	var i User
	err := row.Scan(
		&i.Guid,
		&i.Name,
		&i.Email,
		&i.Occupation,
		&i.IsDeleted,
		&i.CreatedAt,
		&i.UpdatedAt,
	)
	return i, err
}

const insertUser = `-- name: InsertUser :exec
INSERT INTO users (guid, name, occupation, email, created_at, updated_at) VALUES ($1, $2, $3, $4, now(), now())
`

type InsertUserParams struct {
	Guid       uuid.UUID
	Name       string
	Occupation string
	Email      string
}

func (q *Queries) InsertUser(ctx context.Context, arg InsertUserParams) error {
	_, err := q.exec(ctx, q.insertUserStmt, insertUser,
		arg.Guid,
		arg.Name,
		arg.Occupation,
		arg.Email,
	)
	return err
}

const updateUser = `-- name: UpdateUser :exec
UPDATE users SET name = $1, occupation = $2, updated_at = now() WHERE guid = $3
`

type UpdateUserParams struct {
	Name       string
	Occupation string
	Guid       uuid.UUID
}

func (q *Queries) UpdateUser(ctx context.Context, arg UpdateUserParams) error {
	_, err := q.exec(ctx, q.updateUserStmt, updateUser, arg.Name, arg.Occupation, arg.Guid)
	return err
}
